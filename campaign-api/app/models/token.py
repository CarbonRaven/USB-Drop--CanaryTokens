...
class Token(Base):
    ...
    drive_id = Column(UUID(as_uuid=True), ForeignKey("drives.id"), nullable=False, index=True)

    # CanaryTokens API data
    canary_token_id = Column(String(255), nullable=False, unique=True, index=True)
    auth_token = Column(String(255), nullable=True)  # Required for downloads
    token_type = Column(String(50), nullable=False)  # dns, ms_word, ms_excel, etc.
    ...
    short_urls = relationship("ShortUrl", back_populates="token", cascade="all, delete-orphan", order_by="ShortUrl.position")
